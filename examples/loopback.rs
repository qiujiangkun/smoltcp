...
fn main() {
    ...
    let server_handle = socket_set.add_tcp(server_socket);
    let client_handle = socket_set.add_tcp(client_socket);
    ...
    while !done && clock.elapsed() < Instant::from_millis(10_000) {
        ...
        {
            let mut socket = socket_set.get_tcp(server_handle).unwrap();
        ...
        {
            let mut socket = socket_set.get_tcp(client_handle).unwrap();
            if !socket.is_open() {
                if !did_connect {
                    debug!("connecting");
                    socket
