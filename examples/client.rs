...
fn main() {
    ...
    let tcp_rx_buffer = TcpSocketBufferSync::new(vec![0; 64]);
    let tcp_tx_buffer = TcpSocketBufferSync::new(vec![0; 128]);
    ...
    let tcp_handle = sockets.add_tcp(tcp_socket);

    {
        let socket = sockets.get_tcp(tcp_handle).unwrap();
