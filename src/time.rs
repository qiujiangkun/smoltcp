...
impl Instant {
    ...
    pub fn from_micros<T: Into<i64>>(millis: T) -> Instant {
        Instant { micros: millis.into() }
    }

    pub const fn from_micros_const(millis: i64) -> Instant {
...
impl Into<::std::time::SystemTime> for Instant {
    fn into(self) -> ::std::time::SystemTime {
        ::std::time::UNIX_EPOCH + ::std::time::Duration::from_micros(self.micros as u64)
    }
}
